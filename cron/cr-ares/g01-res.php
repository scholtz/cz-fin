...
class MyIter{
	...
	public static function get(){
		...
		if(!MyIter::$res){
            echo "\ngetting res\n";
			MyIter::$res = DB::qb("data_arescz_company_core",["cols"=>["ico","clear"],"where"=>[["col"=>"datumvymazu","op"=>"is","value"=>null]]]);
            var_dump(DB::error());
			MyIter::$limit = DB::num_rows(MyIter::$res);
            var_dump("rows: ".MyIter::$limit."\n");
		}
        
        if(MyIter::$iter%100==0) echo ".";
		if(MyIter::$iter%10000==0) echo MyIter::$iter." ".MyIter::$limit." ".date("c")."\n";
		MyIter::$iter++;

		MyIter::$row=DB::f(MyIter::$res);
		if(!MyIter::$row) return null;
        if(strpos(MyIter::$row["clear"],"v-likvida") !== false){
            return null;
        }
		
		
		$ret = "http://wwwinfo.mfcr.cz/cgi-bin/ares/darv_res.cgi?jazyk=cz&xml=1&ico=".MyIter::$row["ico"];
